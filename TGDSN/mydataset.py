...
def create_dataset_full(data, batch_size, shuffle=True, drop_last=True):
    trainX, testX, trainY, testY = data
    train_dl = DataLoader(MyDataset(trainX, trainY), batch_size=batch_size, shuffle=shuffle, drop_last=drop_last)
    test_dl = DataLoader(MyDataset(testX, testY), batch_size=10, shuffle=False, drop_last=drop_last)
